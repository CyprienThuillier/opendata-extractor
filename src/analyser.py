    ...
    async with sem:
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                address = strip_label(await safe_get(
                    page.locator('[data-item-id="address"]'), attribute="aria-label"
                ))
                website = strip_label(await safe_get(
                    page.locator('[data-item-id="authority"]'), attribute="aria-label"
                ))
                phone = strip_label(await safe_get(
                    page.locator('[data-item-id^="phone"]'), attribute="aria-label", timeout=3000
                ))
